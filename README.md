...
<script>
   ...
   const MASTER_PASS = "bkk123";
    // Masukkan URL Web App (bukan URL Spreadsheet)
    const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxGet86je_vrobTHxk-WeW97bF7972df_nbcQvHNuCdpI0JJf65W_GNmz8ZK438YuUF/exec"; 
    ...
    // --- FUNGSI BARU: RESIZE GAMBAR (Pencegah LocalStorage Penuh) ---
    function compressImage(file, callback) {
        ...
        reader.onload = event => {
            ...
            img.onload = () => {
                ...
                const MAX_WIDTH = 600; // Ukuran optimal untuk laporan
    ...
    document.getElementById('collectionForm').addEventListener('submit', async function(e) {
        ...
        const entri = {
            ...
            gps: document.getElementById('lokasiGps').value || "-7.0,110.0",
            foto: base64Foto
        };
        ...
        syncLabel.innerHTML = "<span class='spinner-border spinner-border-sm'></span> Mengirim ke Google Sheets...";
        ...
        try {
            ...
            syncLabel.innerHTML = "<span class='text-success'>✓ Tersinkron ke Server</span>";
        } catch (err) {
    ...
    function tampilkanData() {
        ...
        const filteredData = dataLaporan.filter(item => item.nama.toLowerCase().includes(searchKeyword));
        ...
        filteredData.forEach(item => {
            ...
            const mapsLink = `https://www.google.com/maps?q=${item.gps}`;
            ...
            row.insertCell(5).innerHTML = `<a href="${mapsLink}" target="_blank" class="btn btn-sm btn-outline-primary"><i class="bi bi-geo-alt"></i></a>`;
        });
    ...
    document.getElementById('collectionForm').addEventListener('submit', async function(e) {
        ...
        try {
            // Gunakan SCRIPT_URL yang benar di atas, atau ganti langsung di sini
            const TARGET_URL = "https://script.google.com/macros/s/AKfycbxdLJdJ3bz6rbwh3IYkK_Ra3N3UbY-KNWYpILEnrg4p4z-pROLaLTie2z9iLUUASQz5/exec";
            
            await fetch(TARGET_URL, {
                method: 'POST',
                mode: 'no-cors', // Mode ini penting untuk Apps Script
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(entri)
            });
